  ...
  $.ajax({
    ...
    success: function (data) {
      $(data)
        ...
        .each(function () {
          persona = {
            imagen: $(this).find("picture").find("medium").text(),
      ...
      // Llamada a gestionImagenes() tras completar el procesamiento de los datos -> Para que gestionImagenes() se ejecute solo después de que se complete la función obtenerDatos(), hay que llamar a gestionImagenes() dentro del bloque de éxito (success) de la petición AJAX en obtenerDatos()
      gestionImagenes();
    },
  });

  // Si pongo gestionImagenes() aquí, se haría antes de que acabase de procesarse toda la obtención de información de la api
}
