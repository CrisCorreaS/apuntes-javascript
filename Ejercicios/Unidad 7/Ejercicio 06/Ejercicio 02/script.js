...
import * as arraysEjercicio1 from "./arrays.js";

let discos = [];

fetch("file/catalogo.xml")
  ...
  .then((docXML) => {
    ...
    for (let i = 0; i < discosXML.length; i++) {
      ...
      disco.cambiarLocalizacion(
        discosXML[i].getElementsByTagName("localizacion")[0].textContent
      );
      disco.cambiarPrestado(
        discosXML[i].getElementsByTagName("prestado")[0].textContent
      );
      disco.cambiarCaratula(
        discosXML[i].getElementsByTagName("caratula")[0].textContent
      );

      discos.push(disco);
    }
  })
  .catch((error) => console.error("Error en la línea:", error));
...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    let disco = new Disco(nombre, grupo, year, tipo);
...
document.getElementById("mostrarTabla").addEventListener("click", function () {
  ...
  discosOrdenados.forEach((disco) => {
    ...
    tabla +=
      `<tr><td>` +
      i +
      `</td><td>${disco.nombre}</td><td>${disco.grupo}</td><td>${disco.year}</td><td>${disco.tipo}</td><td>${disco.localizacion}</td><td>${disco.prestado}</td><td>${disco.caratula}</td></tr>`;
  });
...
document
  .getElementById("mostrarIntervalo")
  .addEventListener("click", function () {
    let intervalo = prompt(
      "Introduzca el intervalo en formato inicio-fin"
    ).split("-");
    let inicio = Number.parseInt(intervalo[0]);
    let fin = Number.parseInt(intervalo[1]);

    let discosFiltrados = discos.filter(
      (disco) => disco.year >= inicio && disco.year <= fin
    );
