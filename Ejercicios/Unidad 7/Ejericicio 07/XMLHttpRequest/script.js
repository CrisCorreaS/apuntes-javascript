...
function obtenerDatosJSON() {
  ...
  xhr.open("GET", "file/pilotos.json", true);
  xhr.onload = function () {
    if (xhr.status == 200 && xhr.readyState == 4) {
      let pilotosJSON = xhr.response;
      pilotos = [...pilotosJSON.pilotos];

      mostrarTodos();
      numeroPaginas = Math.ceil(pilotos.length / pilotosPorPagina);
      paginarPilotos(1);
    } else {
      console.log("Ha habido un problema: " + xhr.statusText);
    }
  };
  xhr.send();
}
...
function paginarPilotos(pagina) {
  ...
  for (let i = (pagina - 1) * pilotosPorPagina; i < pilotosPorPagina * pagina; i++) {
    if (i < pilotos.length){
